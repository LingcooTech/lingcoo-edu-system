...
function TrialCard({ session }: { session: TrialSession }) {
  ...
  if (full) {
    return <div className="pwcard flex flex-col p-5 opacity-70">{inner}</div>;
  }
  return (
    <Link
      to={`/trials/${session.id}`}
      className="pwcard pwcard-hover group flex flex-col p-5 no-underline"
    >
      {inner}
    </Link>
  );
...
function TrialCardSkeleton() {
  return (
    <div className="pwcard flex flex-col p-5">
