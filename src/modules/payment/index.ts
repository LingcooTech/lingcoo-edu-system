...
export const paymentModule: AppModule = {
  ...
  async register(app) {
    ...
    app.post('/public/:tenantSlug/orders', { preHandler: app.authenticateParent }, async (request) => {
    ...
    app.post('/public/payment/wechat/notify', { config: { rateLimit: false } }, async (request, reply) => {
      try {
        ...
        if (notification.kind === 'paid') {
          ...
          request.log.info({ reason: notification.reason }, 'wechat payment notification ignored');
        }
        ...
        return reply
          .status(200)
          .type('text/xml; charset=utf-8')
          .send('<xml><return_code><![CDATA[FAIL]]></return_code><return_msg><![CDATA[FAIL]]></return_msg></xml>');
      }
    });

    app.post('/public/payment/alipay/notify', { config: { rateLimit: false } }, async (request, reply) => {
      try {
